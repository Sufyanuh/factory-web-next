...
const DetailedEvents = (props) => {
  ...
  const { id: stremId } = router.query;
  const { data, loading } = useQuery(GET_SCHEDULED_STREAM_BY_ID, {
    variables: { streamId: Number(stremId) },
  });
  const { data: ShowComments } = useQuery(GET_STREAM_COMMENT, {
    variables: { streamId: Number(stremId) },
  });
            ...
            <div className="lg:rounded-xl shadow-xl bg-secondary relative">
              {data?.getScheduleStreamById?.mediaType === "IMAGE" ? (
                <Image
                  ...
                  src={data?.getScheduleStreamById?.media}
                  alt=""
                  width={500}
                  height={500}
                />
              ...
              {!product ? (
                <>
                  {/* Live Comment */}
                  {ShowComments?.getStreamComments.length > 0 &&
                    ShowComments?.getStreamComments.map((e) => (
                      <div
