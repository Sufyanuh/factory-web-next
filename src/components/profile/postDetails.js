...
import { uploadFileToS3 } from "../../utils/S3singleFileUpload";
      ...
      {!loading ? (
        <div className=" tt relative mx-auto overflow-hidden shadow-xl rounded-lg lg:flex items-center ax-w-[86rem] w-full lg:h-screen">
          ...
          <div className="lg:h-full lg:w-[calc(100vw-400px)] w-full h-96 flex justify-center items-center relative">
            <Helmet>
              <title>{data?.getPostById?.title}</title>
              <meta property="og:title" content={data?.getPostById?.title} />
              <meta
                property="og:description"
                content={data?.getPostById?.title}
              />
              <meta property="description" content={data?.getPostById?.title} />
              <meta
                property="og:image"
                content={
                  data?.getPostById?.attachments.length > 0
                    ? data?.getPostById?.attachments[0].url
                    : undefined
                }
              />
              <meta
              ...
              <meta
                property="og:url"
                content={`https://factoryuniverse.com/Post/${id}`}
              />
