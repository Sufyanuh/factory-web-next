...
async function getDetails(id) {
  ...
  const res = await fetch("https://factolabs.app/graphql", {
    ...
    body: JSON.stringify({
      ...
      variables: {
        productId: Number(id), // Fix: Use consistent variable name here (productId)
      },
    }),
  });
  return res.json();
...
const ProductDetails = (props) => {
  ...
  return (
    <>
      <Head>
        <title>{data?.getProductById?.title?.substring(0, 60)}</title>
        <meta
          name="description"
          content={data?.getProductById?.title?.substring(0, 170)}
        />
        <meta property="og:title" content={data?.getProductById?.title} />
        <meta
          property="og:description"
          content={data?.getProductById?.description}
        />
        {data?.getProductById?.productImages.length !== 0 && (
          <meta
            property="og:image"
            content={data?.getProductById?.productImages?.[0]?.url}
          />
        )}
        <meta
          property="og:url"
          content={`https://factory-web-nextjs.vercel.app/product/${data?.getProductById?.id}`}
        />
        <link
          rel="canonical"
          href={`https://factory-web-nextjs.vercel.app/product/${data?.getProductById?.id}`}
        />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    {moment(Number(data?.getProductById?.createdAt)).isSame(
                      ...
                      ? moment(
                          Number(data?.getProductById?.createdAt)
                        ).fromNow()
